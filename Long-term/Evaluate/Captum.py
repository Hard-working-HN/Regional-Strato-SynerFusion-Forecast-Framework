...
for pq in glob.glob('./data/newparquet/*.parquet'):
    ...
    train_loader, x_tr, _, x_tr_st, _, _, _ = tslib_data_loader_long(
        window, pred_len, batch_size,
        train_data, stamp_train,
        samples_per_county=samples_per_county,
        shuffle=True, start_offset=0
    )
    _, x_va, _, x_va_st, _, _, _ = tslib_data_loader_long(
        window, pred_len, batch_size,
        val_data, stamp_val,
        samples_per_county=samples_per_county,
        shuffle=False, start_offset=train_end
    )
    test_loader, x_te, _, x_te_st, _, _, _ = tslib_data_loader_long(
        window, pred_len, batch_size,
        test_data, stamp_test,
        samples_per_county=samples_per_county,
        shuffle=False, start_offset=val_end
    )
    ...
    lstm.load_state_dict(torch.load(
        f'./long_weights/{file_name}/{file_name}_best_LSTM_model.pth'
    ))
    ...
    star.load_state_dict(torch.load(
        f'./long_weights/{file_name}/{file_name}_best_STAR_model.pth'
    ))
    star.eval()
    star.use_norm = False
