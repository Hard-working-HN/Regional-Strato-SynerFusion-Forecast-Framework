...
weights_root        = "./long_weights"  
output_root         = "./data/other_county_long"
...
def test_model_and_save_results():
    ...
    for subdir in sorted(glob.glob(os.path.join(parquet_test_root, '*'))):
        ...
        for parquet_file in sorted(glob.glob(os.path.join(subdir, '*.parquet'))):
            ...
            for chunk_idx, rb in enumerate(scanner.to_batches()):
                t0_chunk = time.perf_counter()
                df_chunk = rb.to_pandas()
                rows = df_chunk.shape[0]
                n_counties = rows // samples_per_county
                start_c = chunk_idx * chunk_counties + 1
                end_c = start_c + n_counties - 1
                ...
                test_loader, _, _, _, _, _, idxs = tslib_data_loader_long(
                ...
                with torch.no_grad():
                    for i, (feat, yb, feat_mk, yb_mk, lbl, _) in enumerate(test_loader):
                        ...
                        p_star = p_star[:, :, -1] if p_star.dim() == 3 else p_star
                        ...
                        if (i+1) % 10 == 0 or i == len(test_loader)-1:
                            print(f"      已预测批次: {i+1}/{len(test_loader)}")
                ...
                out_file = os.path.join(out_dir, f"{fname}_{start_c}_{end_c}.xlsx")
                df_out = pd.DataFrame({
                    'date': date_col.values[idxs_np],
                    'county_index': (idxs_np // samples_per_county) + 1 + chunk_idx * chunk_counties,
                    'true_value': inv_l,
                    'pred_value': inv_p
                })
                df_out.to_excel(out_file, index=False)

                del df_chunk, data_scaled, data_stamp, test_loader, all_preds, all_lbls
                del preds, lbls, tmp_p, tmp_l, inv_p, inv_l, df_out, df_met
