...
for parquet_file in parquet_files:
    ...
    train_loader, x_train, y_train, x_train_mark, y_train_mark, label_train_lstm, label_indices_train = tslib_data_loader_long(
        window, length_size, batch_size, data_train, data_stamp_train, samples_per_county=samples_per_county, shuffle=True, device_param=device, start_offset=0)

    val_loader, x_val, y_val, x_val_mark, y_val_mark, label_val_lstm, label_indices_val = tslib_data_loader_long(
        window, length_size, batch_size, data_val, data_stamp_val, samples_per_county=samples_per_county, shuffle=False, device_param=device, start_offset=train_size)

    test_loader, x_test, y_test, x_test_mark, y_test_mark, label_test_lstm, label_indices_test = tslib_data_loader_long(
        window, length_size, batch_size, data_test, data_stamp_test, samples_per_county=samples_per_county, shuffle=False, device_param=device, start_offset=val_size)
    ...
    for epoch in range(epochs_phase1):
        ...
        for step, (feature_, _, feature_mark_, _, label_, _) in enumerate(train_loader):
            ...
            feature_mark_ = feature_mark_.to(device)
        ...
        for step, (feature_, y_train_batch, feature_mark_, y_train_mark_batch, label_, _) in enumerate(train_loader):
            ...
            feature_mark_ = feature_mark_.to(device)
            y_train_mark_batch = y_train_mark_batch.to(device)
            label_ = label_.to(device)
        ...
        with torch.no_grad():
            for step, (feature_, y_val_batch, feature_mark_, y_val_mark_batch, label_, _) in enumerate(val_loader):
                ...
                feature_mark_ = feature_mark_.to(device)
                y_val_mark_batch = y_val_mark_batch.to(device)
                label_ = label_.to(device)
    ...
    with torch.no_grad():
        for step, (feature_, y_test_batch, feature_mark_, y_test_mark_batch, label_, label_idx) in enumerate(test_loader):
            ...
            feature_mark_ = feature_mark_.to(device)
            y_test_mark_batch = y_test_mark_batch.to(device)
            label_ = label_.to(device)
