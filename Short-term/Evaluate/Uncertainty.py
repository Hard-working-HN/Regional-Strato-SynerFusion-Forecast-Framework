...
noise_level = 0.000    # 噪声幅度上限
num_samples = 128     # 不确定性样本数

# 不确定性结果保存目录
uncertainty_root = './data/Short_uncertainty'
...
# 批量处理所有 parquet 文件
parquet_files = glob.glob("data/parquettest/*.parquet")
for parquet_file in parquet_files:
    ...
    weights_dir = os.path.join('./short_weights', file_name)
    ...
    for i in range(1, num_samples + 1):
        # Ensure consistent noise across files for the same sample index
        sample_seed = seed + i
