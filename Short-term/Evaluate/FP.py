...
# ----------------- 用户配置 -----------------
output_dir = r"E:\VScode\Article\Com_Model\data\short_pdp_plots"  # 根输出目录
...
def predict_lstm(model, data_inverse, data_stamp, start, length):
    ...
    with torch.no_grad():
        for feat, _, feat_m, _, _, _ in loader:
            p = model(feat, feat_m).squeeze(1)  # [batch]
            preds.append(p.cpu().numpy())
...
for pf in parquet_files:
    ...
    for i, col in enumerate(feats.columns):
        ...
        print(f"特征 [{col}] 完成 ({len(ratios)} 步, 用时 {time.perf_counter()-t0_fg:.1f}s), 保存于 {path}")

    # 清理
    del df, inv, stamp, star, lstm
    torch.cuda.empty_cache()
    gc.collect()
    print(f"{name} 完成，耗时 {time.perf_counter()-t0_file:.1f}s")
