...
for parquet_file in parquet_files:
    ...
    # 根据训练数据确定输入特征数（这里表示每个时间步输入的维度）
    sample_feature = x_train[:1]         # shape: (1, window, feature_dim)
    sample_feature_mark = x_train_mark[:1] # shape: (1, window, time_feature_dim)
    input_size = sample_feature.shape[2] + sample_feature_mark.shape[2]
    hidden_size = 128

    # 定义 3 层 RNN 模型（输出尺寸为 length_size，即预测未来 90 天）
    class SimpleRNN(nn.Module):
        def __init__(self, input_size, hidden_size, output_dim, window, num_layers=3):
            super(SimpleRNN, self).__init__()
            # 定义 RNN 层，使用 nn.RNN，激活函数设为 'relu'
            self.rnn = nn.RNN(input_size=input_size, hidden_size=hidden_size, 
                              num_layers=num_layers, batch_first=True, nonlinearity='relu')
            # 全连接层：将 RNN 所有时间步的隐状态拼接后映射到最终输出
            self.fc = nn.Linear(hidden_size * window, output_dim)
        
        def forward(self, x, x_mark):
            # x: (batch, window, feature_dim)
            # x_mark: (batch, window, time_feature_dim)
            combined = torch.cat((x, x_mark), dim=2)  # 拼接得到 (batch, window, input_size)
            # 通过 RNN 层，忽略隐藏状态（只使用全部时间步的输出）
            out, _ = self.rnn(combined)
            # 将所有时间步的输出展平，形状变为 (batch, window * hidden_size)
            out = out.contiguous().view(out.size(0), -1)
            # 线性映射到最终输出
            out = self.fc(out)
            return out

    # 使用 RNN 模型替换原来的 CNN 模型
    rnn_model = SimpleRNN(input_size, hidden_size, output_dim=length_size, window=window, num_layers=3).to(device)
    print("模型初始化完成：")
    print(rnn_model)

    # 创建权重保存文件夹（保存路径中用 ShortRNNWeights 表示中期模型）
    weights_dir = './ShortRNNWeights'
    ...
    for epoch in range(epochs):
        ...
        for step, (feature_, y_train_batch, feature_mark_, y_train_mark_batch, label_, _) in enumerate(train_loader):
            optimizer.zero_grad()
            pred = rnn_model(feature_, feature_mark_)
        ...
        if val_loss_sum < best_val_loss:
            ...
            torch.save(rnn_model.state_dict(), os.path.join(file_weights_dir, f'{file_name}_best_RNN_model.pth'))
        print(f"[{file_name}] Epoch {epoch+1}/{epochs} - RNN 训练损失: {train_loss_sum:.8f}, 验证损失: {val_loss_sum:.8f}")
    print("——————————————————————3层RNN训练结束——————————————————————")

    # 测试集预测：对测试集每个县单独滑动构造预测序列
    print("——————————————————————测试集预测开始——————————————————————")
    rnn_model.load_state_dict(torch.load(os.path.join(file_weights_dir, f'{file_name}_best_RNN_model.pth')))
    rnn_model.eval()
    ...
    for i in range(test_counties):
        ...
        for start in range(0, n - window + 1, step):
            ...
            if remaining <= 0:
                break
            # L 为本次预测标签长度：若剩余天数超过预测窗口，则预测整个窗口，否则预测剩余部分
            ...
            with torch.no_grad():
                pred_full = rnn_model(x_tensor, x_mark_tensor)  # 输出 shape: (1, length_size)
    ...
    del rnn_model, train_loader, val_loader
